package com.ceep.academia.client.jpql;
...
public class TestJPQL {
    ...
    public static void main(String[] args) {
        ...
        jpql = "select p from Persona p where p.nombre = 'Luis'";
        personas = em.createQuery(jpql).getResultList();
        //mostrarPersonas(personas);
        ...
        System.out.println("\n7. Regresa el valor minimo y maxico del "
                + "idPersona (scaler result)");
        ...
        log.debug("\n11. Obtiene las personas que contengan una letra a en el "
                + "nombre, sin importar si es mayusculas o minuscula");
        ...
        log.debug("\n13. Uso del ordenamiento");
        jpql = "select p from Persona p where p.idPersona > 1 order by p.nombre "
                + "desc, p.apellido desc";
        personas = em.createQuery(jpql).getResultList();
        //mostrarPersonas(personas);
        
        //14. Uso de subquery 
        log.debug("\n14. Uso de subquery");
        jpql = "select p from Persona p where p.idPersona in (select min(p1.idPersona) "
                + "from Persona p1)";
        personas = em.createQuery(jpql).getResultList();
        //mostrarPersonas(personas);
